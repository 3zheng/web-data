...
type Debt struct {
	...
	OrderFormDate time.Time `json:"OrderFormDate"` //欠款订单时间,数据库里这是个datetime类型
...
func GetDebt(db *sql.DB) [](*Debt) {
	...
	for rows.Next() {
		data := new(Debt)
		//其中一个字段的信息 ， 如果要获取更多，就在后面增加：rows.Scan(&row.Name,&row.Id)
		rows.Scan(&data.DebtorName, &data.DebtAmount, &data.CurrencyUnit, &data.OrderFormDate,
...
type Salesman struct {
	...
	SalesDate    time.Time `json:"SalesDate"`    //销售日期
...
func GetSalesman(db *sql.DB) [](*Salesman) {
	...
	for rows.Next() {
		data := new(Salesman)
		//其中一个字段的信息 ， 如果要获取更多，就在后面增加：rows.Scan(&row.Name,&row.Id)
		rows.Scan(&data.SalesDate, &data.Name, &data.SalesAmount, &data.OrderFormNum)
		rowsData = append(rowsData, data)
	}
	return rowsData
}
...
type ImportantCustomer struct {
	...
	Month             time.Time `json:"Month"`             //月开始日期
...
func GetImportantCustomer(db *sql.DB) [](*ImportantCustomer) {
	...
	for rows.Next() {
		...
		//其中一个字段的信息 ， 如果要获取更多，就在后面增加：rows.Scan(&row.Name,&row.Id)
		rows.Scan(&data.CustomerID, &data.CustomerName, &data.Month, &data.ConsumptionAmount,
...
type LostImportantCustomer struct {
	...
	CustomerID         int       `json:"CustomerID"`         //客户ID
	CustomerName       string    `json:"CustomerName"`       //客户姓名
	CurrentMonth       time.Time `json:"CurrentMonth"`       //当前月
	ConsumptionAmount  float64   `json:"ConsumptionAmount"`  //月购买总金额
	ConsumptionTimes   int       `json:"ConsumptionTimes"`   //月购买次数
...
func GetLostImportantCustomeromer(db *sql.DB) [](*LostImportantCustomer) {
	...
	for rows.Next() {
		...
		//其中一个字段的信息 ， 如果要获取更多，就在后面增加：rows.Scan(&row.Name,&row.Id)
		rows.Scan(&data.CustomerID, &data.CustomerName, &data.CurrentMonth, &data.ConsumptionAmount,
